...
print("--- 指定した全品詞の出現頻度 ---")
for word, cnt, num in sorted(words_cnt.iteritems(), key=lambda x: x[1], reverse=True): #, range(word):
    ...
    #dict_sample = {num: word}

    #f = open("okayama-test.json", "w")
...
#結果の出力
#print str(matched_list_decision)
#sys.stdout.flush()

f = open('output-okayama-matched.json', 'w')
json.dump(matched_list_decision, f, indent=4)
